...
export function extractProvider(pathname: string): { host: string | null; rest: string } {
...
function parseTokensFromJson(obj: any): { inputTokens: number | null; outputTokens: number | null; cachedTokens: number | null; totalTokens: number | null; model: string | null } {
  ...
  if (u) {
    ...
    if (typeof u.completionTokens === "number") outputTokens = u.completionTokens
    ...
    if (u.prompt_tokens_details && typeof u.prompt_tokens_details.cached_tokens === "number") cachedTokens = u.prompt_tokens_details.cached_tokens
    if (typeof u.cache_read_input_tokens === "number") cachedTokens = u.cache_read_input_tokens
    if (typeof u.cached_content_token_count === "number") cachedTokens = u.cached_content_token_count
  }
  if (obj.prompt_tokens && inputTokens === null) inputTokens = obj.prompt_tokens
  if (obj.completion_tokens && outputTokens === null) outputTokens = obj.completion_tokens
  if (inputTokens !== null && outputTokens !== null && totalTokens === null) totalTokens = inputTokens + outputTokens
...
export async function handleProxy(request: Request): Promise<Response> {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Upstream fetch failed", target: targetUrl, details: String(e?.message ?? e) }), {
      status: 502,
      headers: { "content-type": "application/json" },
    })
  }

  const isStreaming = (upstream.headers.get("content-type") || "").includes("event-stream") || upstream.headers.get("x-stream") === "1"
  ...
  if (!upstream.body) {
    const latencyMs = Math.round(performance.now() - start)
    let inputTokens: number | null = null, outputTokens: number | null = null, cachedTokens: number | null = null, totalTokens: number | null = null, model: string | null = null
    ...
    if (ct.includes("application/json")) {
      try {
        ...
        if (json) {
          ...
          inputTokens = p.inputTokens; outputTokens = p.outputTokens; cachedTokens = p.cachedTokens; totalTokens = p.totalTokens; model = p.model
        }
        const metricsLatency = Math.round(performance.now() - start)
        await insertMetric({
          provider: host, path: rest, method: request.method, status: upstream.status,
          latencyMs: metricsLatency, ttftMs: null, isStreaming: false,
          inputTokens, outputTokens, cachedTokens, totalTokens,
          requestBytes, responseBytes: text.length, timestamp: new Date(), model, error: upstream.ok ? null : `upstream ${upstream.status}`,
        })
    ...
    await insertMetric({
      provider: host, path: rest, method: request.method, status: upstream.status,
      latencyMs, ttftMs: null, isStreaming: false,
      inputTokens, outputTokens, cachedTokens, totalTokens,
      requestBytes, responseBytes: 0, timestamp: new Date(), model, error: upstream.ok ? null : `upstream ${upstream.status}`,
    })
  ...
  if (!isStreaming && contentType.includes("application/json")) isJsonBuffered = true
      ...
      try {
        while (true) {
          ...
          if (firstByteAt === null) {
            ...
            ttftMs = Math.round(firstByteAt - start)
          }
          responseBytes += value.byteLength
          if (isStreaming || contentType.includes("event-stream") || contentType.includes("text/plain")) {
        ...
        if (isStreaming || sseBuffer) {
          ...
          inputTokens = parsed.inputTokens; outputTokens = parsed.outputTokens; cachedTokens = parsed.cachedTokens; totalTokens = parsed.totalTokens; model = parsed.model
        }
        ...
        if (!isJsonBuffered && !sseBuffer && !isStreaming) {
          ttftMs = firstByteAt !== null ? Math.round(firstByteAt - start) : null
        }
